...
const ManagerAssignment = () => {
  ...
  return <div className="fade-in" style={{ padding: "1rem" }}><div style={{ marginBottom: "2.5rem" }}><h1 style={{ display: "flex", alignItems: "center", gap: "1rem", fontSize: "2.25rem", marginBottom: "0.5rem" }}><Users className="text-primary" size={36} /> Manager Assignment Hub
        </h1><p className="text-muted" style={{ fontSize: "1.1rem" }}>High-level project management, team assignments, and resource allocation.</p></div>{successMessage && <div className="card glass fade-in" style={{
    ...
    borderLeftWidth: "4px"
  }}><CheckCircle2 size={24} /><div style={{ fontWeight: 600, fontSize: "1rem" }}>{successMessage}</div><button
    onClick={() => setSuccessMessage("")}
    style={{ marginLeft: "auto", color: "var(--success)", padding: "0.25rem" }}
  ><X size={20} /></button></div>}<div style={{ marginBottom: "3rem" }}><ManagerDashboard onSuccess={(msg) => {
    setSuccessMessage(msg);
    dispatch(fetchProjects());
    dispatch(fetchUsers());
    window.scrollTo({ top: 0, behavior: "smooth" });
  ...
  /></div></div><div style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>{loading && projects.length === 0 ? <div className="loading-spinner" style={{ margin: "4rem auto" }} /> : filteredProjects.length === 0 ? <div style={{ textAlign: "center", padding: "5rem 2rem", border: "1px dashed var(--border)", borderRadius: "var(--radius)" }}><Briefcase size={48} className="text-muted" style={{ marginBottom: "1rem", opacity: 0.3 }} /><p className="text-muted" style={{ fontSize: "1.1rem" }}>No projects found matching your search.</p></div> : filteredProjects.map((project) => <div
    key={project._id}
  ...
  ><div style={{ display: "flex", gap: "1.5rem", alignItems: "center" }}><div style={{
    padding: "1rem",
    background: "var(--bg-secondary)",
    borderRadius: "12px",
    color: "var(--primary)",
    border: "1px solid var(--border)"
  }}><Briefcase size={24} /></div><div><h4 style={{ margin: "0 0 0.5rem 0", fontSize: "1.2rem", fontWeight: 700 }}>{project.name}</h4><p style={{ margin: 0, fontSize: "0.95rem", color: "var(--text-muted)", maxWidth: "400px" }}>{project.description}</p></div></div><div style={{ display: "flex", gap: "3rem", alignItems: "center" }}><div style={{ textAlign: "center" }}><div style={{ fontSize: "0.75rem", fontWeight: 700, textTransform: "uppercase", color: "var(--text-muted)", marginBottom: "0.5rem", letterSpacing: "0.05em" }}>Team</div><div style={{ display: "flex", alignItems: "center", gap: "0.5rem", justifyContent: "center" }}><div style={{ display: "flex", marginLeft: "0.5rem" }}>{project.members?.slice(0, 3).map((m, i) => <div key={i} style={{
    width: "28px",
    height: "28px",
    borderRadius: "50%",
  ...
  }}>{m.name?.charAt(0) || "U"}</div>)}{(project.members?.length || 0) > 3 && <div style={{ width: "28px", height: "28px", borderRadius: "50%", background: "var(--bg-secondary)", color: "var(--text-muted)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "0.7rem", border: "2px solid var(--bg)", marginLeft: "-8px" }}>
                             +{project.members.length - 3}</div>}</div><span style={{ fontWeight: 600, fontSize: "0.9rem" }}>{project.members?.length || 0}</span></div></div><div style={{ textAlign: "center" }}><div style={{ fontSize: "0.75rem", fontWeight: 700, textTransform: "uppercase", color: "var(--text-muted)", marginBottom: "0.5rem", letterSpacing: "0.05em" }}>Status</div><span className={`status-badge status-${project.status.toLowerCase()}`} style={{ padding: "0.4rem 1rem", borderRadius: "50px" }}>{project.status}</span></div><div style={{ display: "flex", gap: "0.75rem" }}><button
    className="btn btn-outline btn-sm"
    onClick={() => navigate(`/projects/${project._id}`)}
    style={{ padding: "0.6rem 1rem" }}
  >
                        View Details <ArrowRight size={16} style={{ marginLeft: "0.5rem" }} /></button><button
  ...
  ><Trash2 size={18} /></button></div></div></div>)}</div></div>{
  ...
  }}><AlertTriangle size={32} /></div><h3 style={{ marginBottom: "1rem" }}>Delete Project?</h3><p style={{ color: "var(--text-muted)", marginBottom: "2rem", lineHeight: 1.5 }}>
  ...
  >{deleteLoading ? "Deleting..." : "Confirm Delete"}</button></div></div></Modal>{projectToAssign && <AssignMemberModal
...
var stdin_default = ManagerAssignment;
export {
