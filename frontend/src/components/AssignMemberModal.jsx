...
const AssignMemberModal = ({ isOpen, onClose, projectId, currentMembers, requiredSkills = [], onSuccess }) => {
  ...
  return <Modal isOpen={isOpen} onClose={onClose} title="Assign Team Members"><div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>{error && <div className="status-badge status-todo" style={{ width: "100%", padding: "0.75rem", textAlign: "center" }}>{error}</div>}<div style={{ position: "relative" }}><Search size={18} style={{ position: "absolute", left: "1rem", top: "50%", transform: "translateY(-50%)", color: "var(--text-muted)" }} /><input
    type="text"
  ...
  /></div><div style={{ maxHeight: "400px", overflowY: "auto", border: "1px solid var(--border)", borderRadius: "var(--radius)", background: "var(--bg-secondary)", padding: "0.5rem" }}><div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>{filteredUsers.map((user) => {
    const matchedSkills = (user.skills || []).filter(
      (s) => requiredSkills.some((rs) => rs.toLowerCase() === s.toLowerCase())
    );
    return <div
      key={user._id}
    ...
    ><div style={{
      ...
      borderRadius: "4px",
      fontWeight: 600
    }}>{s}</span>)}</div>}</div>{selectedMembers.includes(user._id) && <CheckCircle2 size={28} />}</div>;
  })}</div></div><div style={{ display: "flex", gap: "1rem", justifyContent: "flex-end", paddingTop: "1rem", borderTop: "1px solid var(--border)" }}><button onClick={onClose} className="btn btn-outline">Cancel</button><button
